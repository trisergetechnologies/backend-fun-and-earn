...
async function earnCredit(ctx, beneficiary) {
  ...
  const res = await userPost(ctx, joiner.token, '/pools/1/join', {
    ...
    idempotencyKey: `rcred-${joiner.userId}`,
  });
...
module.exports = {
  id: 'R_release',
  title: 'Release gate — re-entry blocked until upgrade used + 15/15',
  intent: 'Docs §8.1 release; edge case 2 vs not-released',
  async run(ctx) {
    // Path A: complete 15 without upgrade → blocked re-entry
    ...
    await fillCycles(ctx, a, 1, 15);
    const pA = await userGet(ctx, a.token, '/pools/1');
    expectEq(pA.data?.data?.participation?.cycleCount, 15, 'A at 15');
    ...
    const partB = await fillCycles(ctx, b, 1, 15);
    expect(partB.upgradeUsedAt, 'B upgrade used');
    expect(partB.releasedAt, 'B released after 15+upgrade');
    expectEq(partB.cycleCount, 15, 'B at 15');
